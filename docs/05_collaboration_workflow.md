...
**Provided by `database.py` (Ayub Timurov) — called by Bilol Arzykulov / `main.py`:**
...
**Provided by `recognition.py` (Bilol Arzykulov) — called by Ayub Timurov / `main.py`:**
| Function | Input | Output | Purpose |
|---|---|---|---|
...
If either of you needs to change a function's inputs/outputs, say so to the other person first — don't
...
## 4. Suggested Branches per Sprint Task
